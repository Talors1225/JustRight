...
export function renderWishlist() {
  ...
  if (wList.length === 0) {
    var emptyLabel = currentMode === 'game' ? '想玩清单' : '想看清单';
    dom.wishEl.innerHTML = '<div class="panel-label">' + emptyLabel + '</div><div class="panel-empty">暂无收藏</div>';
  } else {
    var label = currentMode === 'game' ? '想玩清单' : '想看清单';
    ...
    var itemsHtml = wList.map(function(g) {
      ...
      if (isMovie) {
        ...
        if (g.type) headerRight += '<span class="wishlist-type">' + (g.type === 'tv' ? '剧集' : '电影') + '</span>';
      }
      ...
      return '<div class="wishlist-item" data-id="' + esc(id) + '">' +
        ...
        '</div>' +
        '<button class="wishlist-remove" data-id="' + esc(id) + '" title="移除">×</button>' +
        '</div>';
    }).join('');
    dom.wishEl.innerHTML = '<div class="panel-label">' + label + ' <span class="wishlist-count">' + wList.length + '</span></div>' +
      ...
      '<button class="btn-wishlist-save"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>保存为图片</button>';
  }
}
  ...
  dom.wishEl.addEventListener('click', function(e) {
    ...
    if (wItem) {
      ...
      if (wGame) {
        ...
        else if (currentMode === 'movie') window.open('https://search.bilibili.com/all?keyword=' + encodeURIComponent(wGame.name), '_blank');
      }
    }
  });
