...
function drawHeader(ctx, W, PAD, title, items, avgScore, isMovie, textPrimary, textTertiary, textAccent, accentColor, logoImg) {
  ...
  ctx.fillText('对味了 · 开源版', PAD, 70);
  if (avgScore > 0) {
    ctx.font = '13px sans-serif';
    ctx.fillStyle = textAccent;
    ctx.fillText('共 ' + items.length + ' 款 · 平均匹配度 ' + avgScore + '%', PAD, 92);
  } else {
    ctx.font = '13px sans-serif';
    ctx.fillStyle = textAccent;
    ctx.fillText('共 ' + items.length + (isMovie ? ' 部' : ' 款'), PAD, 92);
  }
}
...
function drawCard(ctx, item, i, images, opts) {
  ...
  if (item.match_score) {
    ...
    scoreLabel = item.match_score >= 85 ? '强推荐' : item.match_score >= 65 ? '推荐' : '一般';
    ...
    ctx.font = '10px sans-serif';
    ctx.fillStyle = hexToRgba(scoreColor, 0.6);
    ctx.fillText('好评率', W - PAD - 14, scoreY + 14);
  } else if (item.rating) {
    ...
    ctx.font = '10px sans-serif';
    ctx.fillStyle = textTertiary;
    ctx.fillText('评分', W - PAD - 14, scoreY + 14);
  }
  ctx.textAlign = 'left';
}
...
function drawFooter(ctx, W, PAD, footerY, accentColor, watermark, isMovie) {
  ...
  ctx.fillStyle = accentColor;
  ctx.textAlign = 'center';
  ctx.fillText(isMovie ? '影视荒？来试试' : '游戏荒？来试试', W / 2, footerY + 30);
  ctx.font = '11px sans-serif';
  ctx.fillStyle = watermark;
  ctx.fillText('对味了 · 开源版', W / 2, footerY + 52);
  ctx.textAlign = 'left';
}
...
function drawShareCard(title, items, images, W, H, PAD, headerH, queryH, cardH, gap, imgW, imgH, imgPad, footerH, avgScore, logoImg) {
  ...
  if (!ctx) { showToast('生成图片失败，请重试'); return; }
  ...
  canvas.toBlob(function(blob) {
    ...
    a.download = '对味了-' + safeName + '-' + Date.now() + '.png';
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
    showToast('图片已保存，发给朋友试试！');
  }, 'image/png');
}
...
export function generateWishlistImage() {
  ...
  if (items.length === 0) { showToast('暂无收藏'); return; }

  var isMovie = currentMode === 'movie';
  var title = isMovie ? '想看清单' : '想玩清单';
  ...
  function tryDraw() {
    ...
    catch (e) { console.error('生成图片失败:', e); showToast('生成图片失败，请重试'); }
  }
